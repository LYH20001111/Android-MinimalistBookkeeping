...
### 智能记账（V5.0 起，手动记账为主、AI 为辅）
- **扫描账单**：「记一笔」页右上角 `✨` → 扫描账单，拍照或选相册图片（小票 / 消费凭证 / 账单截图），端侧 ML Kit 中文 OCR 离线识别文字，再由 AI 结构化出金额、日期、商家、分类，进入账单确认页。
- **AI 智能记账**：一句话描述消费（如「今天中午吃饭花了32块」），自动解析类型、金额、分类、日期与备注，进入账单确认页。
- **账单确认页**：识别结果**不能直接入账**——类型 / 金额 / 分类 / 账户 / 日期 / 时间 / 备注全部可改，确认后才写入正式账单；识别失败不阻断，随时可回到手动填写。
- **双解析引擎**：可配置 OpenAI 兼容云端模型（「我的」→ AI 服务设置，Base URL / API Key / 模型名，仅存本机）；未配置或断网时自动使用内置离线规则解析，离线全流程可用。
- **隐私**：扫描图片仅用于本次识别，生成草稿后立即删除；不做票据附件存储。
...
### 客户端（Android）
- 语言：Java，minSdk 24 / targetSdk 36，版本 `5.0.0`。
- 架构：`Room` + `ViewModel`/`LiveData` + `ViewBinding`，本地数据实时工作；同步层独立于业务层。
- 同步与网络：`Retrofit` + `OkHttp` + `Gson`，后台同步由 `WorkManager` 调度。
- 模块分布：`data`（数据库 / 实体 / 仓库 / 远程）、`domain`（账户 / 预算 / 周期 / 交易 / 导入导出用例）、`ui`（各功能页与适配器）、`sync`（同步协调器 / 队列 / 协议映射）、`ai`（V5 智能记账：OCR / 解析 / 校验 / 草稿管线）、`util`。
- 智能记账：端侧文字识别 `ML Kit text-recognition-chinese`（模型打包进 APK，离线可用、不依赖 GMS）；云端解析走 OpenAI 兼容 `/chat/completions`（OkHttp 直连，可选配置）。
...
- **V4**：账单**退款**——退款以独立流水记录（待到账 / 已到账 / 已撤销），原账单金额不改写，统计与账户余额改按**净额**口径；同步协议 2→3（退款为第 7 类实体）、Room 8→9、本地备份 5→6、服务器备份 formatVersion 2→3、CSV 导出新增退款两列。
- **V5（当前）**：**智能记账**——「记一笔」右上角新增 `✨` 入口，提供**扫描账单**（端侧中文 OCR + AI 结构化）与**AI 智能记账**（一句话解析账单）两条增强路径，统一产出账单草稿并经**账单确认页**逐字段确认后落库；识别失败自动降级（云端 → 离线规则 → 手动填写），手动记账主流程与既有数据结构完全不变。
